...
(function() {
  ...
  var firstName = displayName ? displayName.split(' ')[0] : null;
  var searchTerm = firstName || '';
  var url = id ? 'https://www.facebook.com/profile/' + id + '/search/?q=' + encodeURIComponent(searchTerm) : null;
  ...
  titleEl.textContent = 'PROFILE SEARCH';
  ...
  if (url) {
    ...
    nameRow.style.cssText = 'padding:8px 0;border-bottom:1px solid #222';
    var nameLbl = document.createElement('span');
    nameLbl.textContent = 'Search';
    nameLbl.style.cssText = 'color:#777';
    var nameVal = document.createElement('span');
    nameVal.textContent = '  ' + searchTerm;
    nameVal.style.cssText = 'color:#fff;font-weight:700';
    ...
    note.style.cssText = 'padding:8px 0;color:#555;font-size:11px;font-style:italic';
    note.textContent = 'Default search is their first name. You can refine your search once the page opens.';
  ...
  if (url) {
    ...
    openBtn.textContent = 'Open Profile Search';
