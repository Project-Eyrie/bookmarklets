...
(function() {
  ...
  // Parses the __NEXT_DATA__ script tag and returns its JSON contents
  ...
  // Extracts the image ID from a Snapchat CDN URL
  ...
  // Creates a label-value row element for the output window
  ...
  if (!nextData) {
    ...
    if (spotlightHighlights.length === 0 && curatedHighlights.length === 0) {
      ...
      // Creates a timestamp badge element and appends it to the given card
      function addBadge(card, epochSec) {
        var old = card.querySelector('._osint_ts');
        if (old) old.remove();
      ...
      for (var si = 0; si < spotlightCards.length; si++) {
        ...
        addBadge(card, parseInt(spotlightTimestampMap[sid]));
      }
      ...
      for (var sti = 0; sti < storyCards.length; sti++) {
        ...
        addBadge(sCard, parseInt(storyThumbnailTimestampMap[imageId]));
      }
